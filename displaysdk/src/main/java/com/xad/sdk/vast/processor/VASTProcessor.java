...
import com.xad.sdk.vast.model.VAST_DOC_ELEMENTS;
import com.xad.sdk.vast.util.XmlTools;
import com.xad.sdk.vast.util.XmlValidation;
...
public final class VASTProcessor {
    ...
    private static final boolean IS_VALIDATION_ON = false; //set true to validate xml with VAST2.0 Schema

    private final VASTMediaPicker mediaPicker;
    private VASTModel vastModel;
    private final StringBuilder mergedVastDocs = new StringBuilder(500);
...
//        xmlData = xmlData.replaceAll("[^\\x20-\\x7e]", "");
        ...
        if (!VASTModelPostValidator.validate(vastModel, mediaPicker)) {
            return VASTPlayer.ERROR_NO_MEDIA_FILE_OR_NO_IMPRESSION;
        }
    ...
    private int processUri(InputStream is, int depth) {
        ...
        if (uriToNextDoc == null || uriToNextDoc.getLength() == 0) {
            ...
            // it.
            Logger.logDebug(TAG, "Doc is a wrapper. ");
            Node node = uriToNextDoc.item(0);
            String nextUri = XmlTools.getElementValue(node);
            Logger.logDebug(TAG, "Wrapper URL: " + nextUri);
    ...
    private Document createDoc(InputStream is) {
        Logger.logDebug(TAG, "About to create doc from InputStream");
        try {
            Document doc = DocumentBuilderFactory.newInstance()
                    .newDocumentBuilder().parse(is);
            doc.getDocumentElement().normalize();
            Logger.logDebug(TAG, "Doc successfully created.");
            return doc;
        } catch (Exception e) {
            Logger.logError(TAG, e.getMessage(), e);
            return null;
    ...
    private void merge(Document newDoc) {
        Logger.logDebug(TAG, "About to merge doc into main doc.");
        ...
        Logger.logDebug(TAG, "Merge successful.");
    }
    
    // Validator using mfXerces.....
    private boolean validateAgainstSchema(Document doc) {
        Logger.logDebug(TAG, "About to validate doc against schema.");
//        InputStream stream = VASTProcessor.class.getResourceAsStream("/assets/vast_2_0_1_schema.xsd");
        InputStream stream = null;
        try {
            stream = Assets.inputStreamFromAssets(DisplaySdk.sharedInstance().getApplicationContext(), "vast_2_0_1_schema.xsd");
        } catch (IOException e) {
            e.printStackTrace();
        }
